"""Feedback of the information retrieval system to obtain a better query"""
import json
...
class RocchioAlgorithm:
    def __init__(self, query: str, corpus: Corpus, relevant_docs: List[int], non_relevant_docs: List[int]):
        ...
        self.query_vect = QueryProcessor().get_query_vector(query, corpus.index)
    ...
    def save_query(query: str, query_vect: List[float], name='misc'):
        try:
            queries = json.load(open(f'../../data/queries/{name}_queries.json', 'r+'))
        except FileNotFoundError:
            queries = {}
        queries[query] = query_vect
        json.dump(queries, open(f'../../data/queries/{name}_queries.json', 'w+'))
